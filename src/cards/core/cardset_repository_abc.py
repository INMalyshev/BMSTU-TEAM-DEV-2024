...
class CardsetRepositoryABC(ABC):
    ...
    def get_cardset_infos(
        ...
        :param cardset_id: ID набора карточек. Если передано, остальные параметры 
        не учитываются и возвращается только набор карточек с соответствующим ID 
        (если он существует).
        :type cardset_id: str, optional
        :param user_id: ID пользователя. Если передано, то вернутся наборы карточек, 
        которые принадлежат пользователю с указанным ID.
        :type user_id: str, optional
        :param offset: Параметр пагинации для результата, указывает с какой записи 
        начинать выборку.
        :type offset: int, optional
        :param limit: Параметр пагинации для результата, указывает максимальное 
        количество записей в выборке.
        :type limit: int, optional
        :param include_deleted: Если True, в результирующей выборке могут оказаться 
        наборы карточек, которые были отмечены как удаленные.
        :type include_deleted: bool, optional
        :return: Выборка укороченных (без карточек) представлений наборов карточек, 
        отсортированная по названию в алфавитном порядке.
    ...
    def get_cards(
        ...
        в результирующей выборке могут оказаться карточки, которые были отмечены как удаленные.
