...
class CardsetRepositoryABC(ABC):
    ...
    def get_cardset_infos(
        ...
        :param offset: Параметр пагинации для результата, указывает с какой записи начинать выборку.
        :type offset: int, optional
        :param limit: Параметр пагинации для результата, указывает максимальное количество записей в выборке.
        ...
        :return: Выборка укороченных (без карточек) представлений наборов карточек, отсортированная по названию в алфавитном порядке.
    ...
    def get_cards(
        ...
        :param card_id: id карточки. Если передано, остальные параметры игнорируются, а в результирующей выборке будет находиться объект с указанным id (если он существует).
        :type card_id: Optional[str], optional
        :param cardset_id: id набора карточек. Если передано, то вернуться карточки, которые принадлежат набору карточек с переданным id (если они существуют).
        ...
        :param include_deleted: Если выставить данный параметр в True, в результирующей выборке могут оказаться карточки, которые были отмечены как удаленные.
        :type include_deleted: Optional[bool], optional
        :param mixed: Если выставить данный параметр в True, в результирующей выборке карточки будут представлены в случайном порядке.
    ...
    def create_cardset_info(
        ...
        """
        Метод create_cardset_info создает набор карточек (без карточек) с учетом переданных данных.
        ...
        :return: Укороченное (без карточек) представление набора карточек или None, если создание набора карточек не было корректно выполнено.
    ...
    def modify_cardset_info(
        ...
        :type spec: CardsetInfoSpec
        :return: Укороченное (без карточек) представление набора карточек или None, если изменение набора карточек не было корректно выполнено.
        :rtype: CardsetInfo | None
        """
        raise NotImplementedError()
